package com.cokreates.grp.filter;
...
public class ResponseFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse res, FilterChain chain) throws IOException, ServletException {
		...
		CharResponseWrapper wrappedResponse = new CharResponseWrapper((HttpServletResponse) response);
  		...
  		if (StringUtils.contains(apiName, "com-acq") && METHOD_POST.equals(httpRequest.getMethod())) {
  			
  			String token = httpRequest.getHeader("Authorization").replace("Bearer ", "");
  			
  			boolean result = isAuthenticate(token);
  			
  			if(!result) {
  				response.sendError(HttpServletResponse.SC_UNAUTHORIZED, "Access denied for this user");
  				log.warn("Trying {} resource with invalid token - {}", apiName, token);
  				return;
  			}
	...
	private boolean isAuthenticate(String token) {
		...
		try {
			...
			j.put("token", token);
			AuthWsResonse response = authWsClient.checkToken(j.toString());
			log.info("Token validation response : {}", response);
			if(response != null && response.getStatus() == 200) {
				result = true;
			}
		} catch (Exception e) {
			log.error("An exception occurred while check token : ", e);
		}
		return result;
	}
