...
public class GetListApprovalQuery {
	...
	@Synchronized
    public static ImmutablePair<String, Object[]> getDirectInSql(GetListApprovalRequest request, AuthUser user ) {
        
    	List<Object> param = Lists.newArrayList(user.getOfficeOid(),user.getUserOid(), Constant.DI_PENDING);
    	
        String sql = "select t.oid as oid, t.code as code, t.status as status, t.requested_by as requestedBy  "
    ...
    public static ImmutablePair<String, Object[]> getDataCountSql(GetListApprovalRequest request, AuthUser user) {
        ...
        String sql = "select count(t.oid)"
            + " from " + Table.SCHEMA_AST + Table.DIRECT_IN + " t"
