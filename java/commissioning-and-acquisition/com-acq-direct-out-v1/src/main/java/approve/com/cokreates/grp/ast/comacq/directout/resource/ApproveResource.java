...
import com.cokreates.grp.ast.comacq.directout.response.ApproveResponseBody;
import com.cokreates.grp.ast.comacq.directout.service.ApproveService;
...
@Slf4j
@RestController("comAcqDirectOutV1ApproveResource")
...
public class ApproveResource {
	...
	@Autowired
	@Qualifier("comAcqDirectOutV1ApproveService")
	private ApproveService service;

	// @HystrixCommand(fallbackMethod = "fallbackGetAllOnPost")
	@PostMapping(path = Api.COMACQ_DIRECTOUT_V1_APPROVE_PATH, produces = { MediaType.APPLICATION_JSON_VALUE }, consumes = { MediaType.APPLICATION_JSON_VALUE })
	public @ResponseBody ApproveResponse onPost(@Valid @RequestBody ApproveRequest request, @RequestHeader(value="Authorization") String token) throws AppException {
		String url = Api.COMACQ_DIRECTOUT_V1_RESOURCE.concat(Api.COMACQ_DIRECTOUT_V1_APPROVE_PATH);
