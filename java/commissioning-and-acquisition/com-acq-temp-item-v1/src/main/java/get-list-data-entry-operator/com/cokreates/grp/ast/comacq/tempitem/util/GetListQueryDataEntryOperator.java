...
public class GetListQueryDataEntryOperator {
	
	@Synchronized
    public static ImmutablePair<String, Object[]> getTemporaryItemSql(AuthUser user, GetListRequestDataEntryOperator request) {
        
    	List<Object> param = Lists.newArrayList(user.getOfficeOid(), user.getUserOid());
    	
        String sql = "select t.oid as tempOid, t.code as tempCode, t.received_at as receivedAt, t.procurement_method as procurementMethod, "
        		   ...
        		   + " t.updated_on as updatedOn from " + Table.SCHEMA_AST + Table.TEMP_ITEM
                   + " t left join " + Table.SCHEMA_CMN + Table.VENDOR +" v on t.vendor_oid = v.oid"
                   + " where 1=1 and t.office_oid = ? and t.received_by = ?" ;
        ...
        if(StringUtils.isNotBlank(request.getBody().getSearchParams().getSearchText())){
    		sql += " AND (LOWER(t.code) like '%" + request.getBody().getSearchParams().getSearchText().trim().toLowerCase()+ "%' OR"
                ...
                + " LOWER(v.name_bn) like '%" + request.getBody().getSearchParams().getSearchText().trim().toLowerCase() + "%')";
        }
    ...
    @Synchronized
    public static ImmutablePair<String, Object[]> getDataCountSql(AuthUser user,GetListRequestDataEntryOperator request) {
